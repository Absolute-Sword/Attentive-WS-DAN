...
def input_visual(imgs, boxes, labels):
    ...
    for i in range(input_boxes.shape[0]):
        plt.gca().add_patch(Rectangle(input_boxes[i][[1, 0]], w[i], h[i], fill=False, edgecolor='r'))
        plt.text(input_boxes[i][1], input_boxes[i][0],
                 dv.VOC_BBOX_LABEL_NAMES[labels.reshape(-1, len(input_boxes))[0][i]])
    plt.axis("off")
    plt.show()


def res_visual(copy_image=None, copy_bboxes=None, copy_labels=None, paste_image=None, paste_bboxes=None,
               paste_labels=None, crop_image=None, drop_image=None, attention_map=None, center=None, input_image=None,
               input_bboxes=None, input_labels=None, features=None, predict=True, model=None):
    ...
    if input_image is not None:
        ...
        for i in range(input_bboxes.shape[0]):
            ...
            ax[1][2].text(input_bboxes[i][1], input_bboxes[i][0],
                          dv.VOC_BBOX_LABEL_NAMES[input_labels.reshape(-1, len(input_bboxes))[0][i]])
