...
from utils.visual import input_visual
...
from cutout.process import cutout_process
import resource
import logging
import time
import warnings
...
run_name = 'train for backbone 1 data cutout'
...
def train(**kwargs):
    ...
    for epoch in range(opt.epoch):
        ...
        for ii, (imgs, boxes, labels, scale) in tqdm(enumerate(dataloader)):
            ...
            imgs, boxes, labels = imgs.cuda().float(), boxes.cuda(), labels.cuda()
            ...
            paste_cache = [None] * 3
            losses, info, *_ = trainer.train_step(imgs, scale, None, *copy_cache, *paste_cache, cutmix_flag,
                                                  plot_flag)
        ...
        logging.info(f"[Batch: {epoch}] eval loss: {eval_result['map']:.4f} lr:{lr_:.3f}")  # 注意这里是会进行四舍五入
